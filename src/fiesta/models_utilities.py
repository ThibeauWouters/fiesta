...
BU2023YE_PARAMETER_NAMES = ["log10_mej_dyn",
                            "vej_dyn",
                            "Yedyn",
                            "log10_mej_wind",
                            "vej_wind",
...
def extract_Bu2023Ye_parameters(filename: str) -> np.array:
    """
    Extract the parameter values from the filename of a Bulla file

    Args:
        filename (str): Bu2032Ye filename
    ...
    rr = [
        np.abs(float(x))
            for x in re.findall(
                r"\d*\.\d+|\d+", filename.removeprefix('nph1.0e+06_')
        )
    ]
...
BULLA_PARAMETER_NAMES = {"Bu2022Ye": BU2022YE_PARAMETER_NAMES,
                         "Bu2023Ye": BU2023YE_PARAMETER_NAMES,
...
EXTRACT_PARAMETERS_FUNCTIONS = {"Bu2022Ye": extract_Bu2022Ye_parameters,
                                "Bu2023Ye": extract_Bu2023Ye_parameters,
